          ...
          <>
            ...
            <div className="mt-5 rounded-2xl border border-gray-100 overflow-hidden">
              <div className="flex items-center justify-between px-4 py-3">
                <div>
                  <p className="text-sm font-bold text-gray-800">食事リマインダー</p>
                  <p className="text-[11px] text-gray-400 mt-0.5">
                    {notifPermission === 'denied'
                      ? 'ブラウザの設定で通知を許可してください'
                      : REMINDER_SLOTS.map((s) => s.time).join(' / ')}
                  </p>
                </div>
                <button
                  type="button"
                ...
                >
                  <span
                    ...
                    style={{ transform: notifEnabled ? 'translateX(20px)' : 'translateX(0)' }}
                  />
                </button>
              </div>
              {notifEnabled && (
                <div className="border-t border-gray-100 px-4 py-2 flex flex-col gap-1">
                  {REMINDER_SLOTS.map((s) => (
                    <div key={s.time} className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-gray-600 font-medium">{s.time}</span>
