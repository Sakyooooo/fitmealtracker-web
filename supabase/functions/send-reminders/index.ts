// FitMealTracker: 食事リマインダーの Web Push 送信
//
// pg_cron(migration 014) から JST 07:00/12:00/18:00 にそれぞれ
// category を指定して起動される。アプリを開いていなくても届く。
//
// 呼び出し例:
//   POST /functions/v1/send-reminders
//   headers: { "x-cron-secret": "<CRON_SECRET>" }
//   body:    { "category": "朝食" }
//   body:    { "category": "朝食", "dryRun": true }  // 送信せず対象者一覧のみ返す
//
...
Deno.serve(async (req) => {
  ...
  let body: { category?: MealCategory; dryRun?: boolean };
  try {
    body = await req.json();
  } catch {
  ...
  if (!category || !(category in REMINDER_LABEL)) {
  ...
  const alreadyRecorded = new Set((mealRows ?? []).map((r) => r.user_id as string));
  ...
  const targets = (subs ?? []).filter((s) => !alreadyRecorded.has(s.user_id as string));

  if (dryRun) {
    return new Response(JSON.stringify({
      category, today, totalSubscriptions: subs?.length ?? 0, targetCount: targets.length,
  ...
  return new Response(JSON.stringify({
    category, today, targetCount: targets.length, sent, staleRemoved: staleIds.length, errors,
  }), { headers: { 'content-type': 'application/json' } });
});
