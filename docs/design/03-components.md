...
**HIG のシート作法**（09-apple-hig.md §5.3）

- **一度に 1 枚だけ。**シートからシートを開かない。開く必要があるなら先に閉じる
- **「保存」を置くなら必ず「キャンセル」も置く。**保存せずに閉じる道を必ず残す
- **モーダルの中に階層を作らない。**「アプリの中のアプリ」にしない。
  多段が必要なら、単一の経路にして「戻る」と「閉じる」を取り違えないようにする
- モーダルはタブバーを覆ってよい（一時的で自己完結だから）
- 複雑・長いフローはシートではなく**全画面**にする
...
- 「**起動直後にアラートを出さない。**キャッシュ済みデータ + 控えめなラベルで伝える」
- タイトルは状況を具体的に書く。「エラー」だけのタイトルにしない

→ **「記録しました」の成功トーストも、原則不要。**保存されたことは
リストに行が増えることで伝わる。トーストを出すのは Undo を提供するときだけでよい。
...
**HIG のタブバー作法**（09-apple-hig.md §5、Tab bars）

- **タブバーはナビゲーション専用。**「現在の画面に対する操作」をタブバーに置かない（それはツールバー）
- **どの画面に移動してもタブバーを見せ続ける。**隠すと現在地が分からなくなる。
  例外はモーダルが覆っているとき
- **オーバーフロー（「その他」タブ）を作らない。**隠れたタブは気づかれず到達もされない。
  4 タブという現行構成はこの点で正しい
- タブバーは**コンテンツの上の層に浮く**。コンテンツは端まで伸ばし、
  バーの高さ + `env(safe-area-inset-bottom)` 分をコンテンツ側の下パディングで確保する
